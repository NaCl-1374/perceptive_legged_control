...
Eigen::Isometry3d getTransformToTargetFrame(const std::string& sourceFrame, const ros::Time& time) {
  ...
  try {
    transformStamped = tfBuffer.lookupTransform(targetFrameId_, sourceFrame, time);
...
int main(int argc, char *argv[])
{
  ...
  ros::Subscriber sub = nodeHandle.subscribe("/ground_truth/state", 1000, odomCallback);
  ...
  while (ros::ok())
  {
    ...
    try
    {
      ...
      if (tfBuffer.canTransform("odom", "base", timeStamp, &errorMsg))
      {

        transformStamped = tfBuffer.lookupTransform("odom", "base", ros::Time(0));

    
        ros::Time timeStamp = ros::Time(0);  // Use Time(0) to get the latest transform.
        if (tfBuffer.canTransform("world", gridMap.getFrameId(), timeStamp, &errorMsg)) {
          const auto transform = getTransformToTargetFrame(gridMap.getFrameId(), timeStamp);
          gridMap = grid_map::GridMapCvProcessing::getTransformedMap(std::move(gridMap), transform, elevationLayer_, targetFrameId_);
        } else {
        ...
        // grid_map::Position submapPosition(positionx, positiony);
        if (debug)
