...
namespace legged
{
  ...
  constexpr size_t GENCOORDNUM = BASE_COORDINATE_SIZE + JOINT_COORDINATE_SIZE;

  // measuredRbdState_ =[euler pos jpos omega vel jvel]
  ...
  public:
    ...
    StateEstimateBase(const kinematic_model_t &eeKinematics);
    ...
    size_t getMode() { return switched_model::stanceLeg2ModeNumber(contactFlag_); }
  ...
  {
    Eigen::Matrix<SCALAR_T, 3, 1> eulerAnglesXYZ = switched_model::eulerAnglesFromQuaternionBaseToOrigin<SCALAR_T>(q);
    ocs2::makeEulerAnglesUnique<SCALAR_T>(eulerAnglesXYZ);
