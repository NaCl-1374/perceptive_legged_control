...
namespace legged
{
  ...
  StateEstimateBase::StateEstimateBase(const kinematic_model_t &eeKinematics)
      : eeKinematics_(eeKinematics.clone()),
        rbdState_(vector_t ::Zero(2 * GENCOORDNUM))
  {
    ros::NodeHandle nh;
  ...
  {
    rbdState_.segment(6, JOINT_COORDINATE_SIZE) = jointPos;
    rbdState_.segment(6 + GENCOORDNUM, JOINT_COORDINATE_SIZE) = jointVel;
  }
                                    ...
                                    const matrix3_t &angularVelCovariance, const matrix3_t &linearAccelCovariance)
  {
    ...
    // vector3_t eulerAnglesXYZ = switched_model::eulerAnglesFromQuaternionBaseToOrigin<scalar_t>(quat);
    // ocs2::makeEulerAnglesUnique(eulerAnglesXYZ);
    vector3_t eulerAnglesXYZ(quatToXyz(quat));
    Eigen::Matrix3d R1 = switched_model::rotationMatrixBaseToOrigin(eulerAnglesXYZ);
    Eigen::Matrix3d R2 = quat.toRotationMatrix();
    Eigen::MatrixXd diff = R1 - R2;
