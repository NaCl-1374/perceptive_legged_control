...
int main(int argc, char* argv[]) {
  ...
  const std::string ocs2_unitree = path.substr(0, path.find_last_of("/")) + "/../../";
  const std::string taskFolder = ocs2_unitree + "ocs2_unitree_loopshaping_mpc/config/";
  const std::string terrainFolder = ocs2_unitree + "ocs2_unitree_loopshaping_mpc/data/";

  const bool use_ros = false;

  // std::string urdfString = getUrdfString(unitree::UnitreeModel::Aliengo);
  std::string urdfString ;

  std::string configName = "unitree";
  const std::string robotName = "unitree";

  ros::init(argc, argv, "unitree_perceptive_mpc_demo");
  ros::NodeHandle nodeHandle;

  ros::param::get("ocs2_unitree_description", urdfString);
  ...
  perceptionConfig.preprocessingParameters =
      convex_plane_decomposition::loadPreprocessingParameters(nodeHandle, "/ocs2_unitree_loopshaping_mpc_perceptive_demo/preprocessing/");
  perceptionConfig.contourExtractionParameters = convex_plane_decomposition::loadContourExtractionParameters(
      nodeHandle, "/ocs2_unitree_loopshaping_mpc_perceptive_demo/contour_extraction/");
  perceptionConfig.ransacPlaneExtractorParameters = convex_plane_decomposition::loadRansacPlaneExtractorParameters(
      nodeHandle, "/ocs2_unitree_loopshaping_mpc_perceptive_demo/ransac_plane_refinement/");
  perceptionConfig.slidingWindowPlaneExtractorParameters = convex_plane_decomposition::loadSlidingWindowPlaneExtractorParameters(
      nodeHandle, "/ocs2_unitree_loopshaping_mpc_perceptive_demo/sliding_window_plane_extractor/");
  perceptionConfig.postprocessingParameters =
      convex_plane_decomposition::loadPostprocessingParameters(nodeHandle, "/ocs2_unitree_loopshaping_mpc_perceptive_demo/postprocessing/");

  auto unitreeInterface = unitree::getUnitreeLoopshapingInterface(urdfString, taskFolder + configName);
  const ocs2::vector_t initSystemState = unitreeInterface->getInitialState().head(switched_model::STATE_DIM);

  // ====== Scenario settings ========
  ocs2::scalar_t forwardVelocity{0.0};
  nodeHandle.getParam("/forward_velocity", forwardVelocity);
  ocs2::scalar_t gaitDuration{0.8};
  ocs2::scalar_t forwardDistance{3.0};configName
  nodeHandle.getParam("/forward_distance", forwardDistance);
  ...
  const auto& baseToHipInBase = unitreeInterface->getKinematicModel().baseToLegRootInBaseFrame(0);
  ...
  auto referenceManager = unitreeInterface->getQuadrupedInterface().getSwitchedModelModeScheduleManagerPtr();
  ...
  switch (unitreeInterface->modelSettings().algorithm_) {
    case switched_model::Algorithm::DDP: {
      ...
      mpcPtr = getDdpMpc(*unitreeInterface, mpcSettings, ddpSettings);
      break;
    }
    case switched_model::Algorithm::SQP: {
      mpcPtr = getSqpMpc(*unitreeInterface, mpcSettings, sqpSettings);
      break;
    }
  }
  ocs2::MPC_MRT_Interface mpcInterface(*mpcPtr);

  std::unique_ptr<ocs2::RolloutBase> rollout(unitreeInterface->getRollout().clone());

  // ====== Execute the scenario ========
  ocs2::SystemObservation observation;
  observation.time = initTime;
  observation.state = unitreeInterface->getInitialState();
  observation.input.setZero(switched_model_loopshaping::INPUT_DIM);
  // ROS_INFO_STREAM("observation.state .size "<< observation.state.size());
  // ROS_INFO_STREAM("observation.input .size "<< observation.input.size());
  ...
  const auto closedLoopSystemSolution =
      ocs2::loopshapingToSystemPrimalSolution(closedLoopSolution, *unitreeInterface->getLoopshapingDefinition());
  ...
  // ====== Visualize ==========
  QuadrupedVisualizer visualizer(unitreeInterface->getKinematicModel(), unitreeInterface->getJointNames(), unitreeInterface->getBaseName(),
                                 nodeHandle);
