...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh) {
  ...
  logFileName_="/tmp/legged/QuadrupedGazeboNodeLog.txt";
...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  // auto mpcJvel=ocs2::centroidal_model::getJointVelocities(optimizedState,leggedInterface_->getCentroidalModelInfo());
  // auto TorMpc=jac.transpose()*ocs2::centroidal_model::getContactForces(optimizedInput,4,leggedInterface_->getCentroidalModelInfo());;// mpc力矩
  // std::cout<<TorMpc<<std::endl;

  // vector_t combined=vector_t::Zero(additionalColumns_.size());
  // combined<<mpcJpos,mpcJvel,TorMpc.tail(12),posDes,velDes,torque;
