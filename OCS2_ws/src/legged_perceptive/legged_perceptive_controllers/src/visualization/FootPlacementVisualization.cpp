  ...
  if (observation.time - lastTime_ > minPublishTimeDifference_) {
    ...
    for (int leg = 0; leg < numFoot_; ++leg) {
      ...
      for (int k = 0; k < middleTimes.size(); ++k) {
        ...
        const auto convexRegion = convexRegionSelector_.getConvexPolygon(leg, observation.time);
        ...
        textMarker.header=header;
        textMarker.id = i;
        textMarker.color = getColor(Color::red, 1);
        textMarker.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
        textMarker.ns = "footPos";
        textMarker.scale.z = 0.02;
        textMarker.pose.position = getPointMsg(projection.positionInWorld);
        textMarker.pose.position.z += 0.015;
        textMarker.pose.orientation = getOrientationMsg({1, 0, 0, 0});
        textMarker.text = std::to_string(projection.positionInWorld[0]) + '\n' +
                          std::to_string(projection.positionInWorld[1]) + '\n' +
                          std::to_string(projection.positionInWorld[2]);
        makerArray.markers.push_back(textMarker);
