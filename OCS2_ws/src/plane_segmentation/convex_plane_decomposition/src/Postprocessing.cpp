 ...
 * @LastEditTime: 2024-03-10 20:50:03
 * @FilePath: /leg_perceptive_ws/src/plane_segmentation/convex_plane_decomposition/src/Postprocessing.cpp
...
void Postprocessing::addSmoothLayer_custom(grid_map::GridMap& gridMap, const Eigen::MatrixXf& elevationData,
                                    const Eigen::MatrixXf& planarityMask) const {
  auto kernelSizeInPixels = [res = gridMap.getResolution()](double realSize) {
    return 2 * static_cast<int>(std::round(realSize / res)) + 1;
  };
  ...
  int kernelSize = std::max(1, std::min(49, 49));  // must be 1, 3 or 5 for current image type, see doc of cv::medianBlur
  grid_map::smoothing::median(gridMap, "elevationWithNaN_i", "elevationWithNaNdmedian", kernelSize, 0, 3);


  gridMap.add("deltah", (gridMap.get("elevationWithNaN_i") - gridMap.get("elevationWithNaNdmedian")));


  Eigen::MatrixXf mij = gridMap.get("deltah").unaryExpr([](double x) { return (x < 0 || x > 0) ? 1 : 0; }).cast<float>();
  Eigen::MatrixXf mij_inv = gridMap.get("deltah").unaryExpr([](double x) { return (x < 0 || x > 0) ? 0 : 1; }).cast<float>();


  Eigen::MatrixXf result = gridMap.get("elevationWithNaNClosed") .cwiseProduct(mij);
  gridMap.add("elevationWithNaNClosedMask", result);
  ...
  grid_map::processing::applyKernelFunction(
      gridMap, "elevationWithNaNClosedMask", "elevationWithNaNClosedDilated", dilationSize,
      [&](const Eigen::Ref<const grid_map::GridMap::Matrix>& data) -> float { return (data - offsets).maxCoeffOfFinites(); });

  gridMap["elevationWithNaNClosedDilated"]=gridMap.get("elevationWithNaNClosedDilated")+gridMap.get("elevationWithNaNClosed").cwiseProduct(mij_inv);
  // Convert to openCV
  auto elevationWithNaNImage = toCv(gridMap.get("elevationWithNaNClosedDilated"));
