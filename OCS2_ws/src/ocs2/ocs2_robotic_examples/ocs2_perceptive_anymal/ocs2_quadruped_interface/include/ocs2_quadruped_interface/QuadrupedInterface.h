...
namespace switched_model
{
  ...
  public:
    ...
    void addProblemaCost(std::string name, std::unique_ptr<ocs2::StateInputCost> term)
    {
      problemPtr_->costPtr->add(name, std::move(term));
    }
      void addProblemaCost(std::string name, std::unique_ptr<ocs2::StateCost> term)
    {
      problemPtr_->stateCostPtr->add(name, std::move(term));
    }
