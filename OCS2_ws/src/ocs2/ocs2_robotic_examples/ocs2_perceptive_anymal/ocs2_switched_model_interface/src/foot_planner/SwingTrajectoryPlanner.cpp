                                                                          ...
                                                                          scalar_t finalTime) const
  {
    ...
    for (const auto &contactPhase : contactTimings)
    {
      if (hasStartTime(contactPhase))
      {
        ...
        const auto desiredBaseOri = getOrientation(desiredBasePose); // xyz euler angle
        scalar_t offset = tan(-desiredBaseOri(1)) * 0.4; // 处理斜坡地形，从腿竖直与坡面到腿沿着重力方向
        vector3_t offsetVector(offset * cos(-desiredBaseOri(1)), 0, offset * sin(-desiredBaseOri(1)));
        matrix3_t R;
        scalar_t z = desiredBaseOri(2);
        R << cos(z), -sin(z), 0, // clang-format off
             sin(z), cos(z), 0,
             0, 0, 1; // clang-format on
        offsetVector = -R.transpose() * offsetVector;

        referenceFootholdPositionInWorld += offsetVector;
